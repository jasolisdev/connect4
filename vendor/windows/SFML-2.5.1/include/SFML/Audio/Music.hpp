...
public:
  ...
  /// timePoints.length are [0, Dur) and (0, Dur-offset] respectively, where Dur
  /// is the value returned by getDuration(). Note that the EOF "loop point"
  /// from the end to the beginning of the stream is still honored, in case the
  /// caller seeks to a point after the end of the loop range. This function can
  /// be safely called at any point after a stream is opened, and will be
